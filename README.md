...
В MDX доступны виджеты `<RollLevelDemo />`, `<PercentTokenDemo />`, `<MementoRollLab />` (страница `/dev/memento-roll`: формулы, эмуляция, график и таблицы ожиданий) и `<MermaidDiagram definition={...} />` (через `MdxShell` или импорт из `@/widgets/...` при необходимости).
